...
class FindPWViewController: UIViewController, IndicatorInfoProvider {
...
//    }
    ...
    func setUI(){
        ...
        tfEmail.addLeftPadding()
        tfEmail.addTarget(self, action: #selector(FieldDidChange), for: .editingChanged)
...
//        print(valueGender)
    ...
    @objc func FieldDidChange() {
        if (tfName.text!.count * tfBirth.text!.count * tfEmail.text!.count) != 0 {
